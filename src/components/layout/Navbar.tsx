      ...
      <div className="max-w-7xl mx-auto px-6 h-14 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-2.5">
          ...
          <div className="leading-tight">
            <div className="text-sm font-bold text-gray-900">WEConnect</div>
            <div className="text-[10px] text-gray-400">Certification Platform</div>
          </div>
        </Link>
          ...
          {links.map(({ href, label, icon: Icon }) => (
            <Link key={href} href={href}
              className={cn("flex items-center gap-1.5 text-sm font-medium px-3 py-1.5 rounded-lg transition-all",
                path.startsWith(href) ? "bg-brand-indigo/10 text-brand-indigo" : "text-gray-600 hover:bg-gray-50 hover:text-gray-900")}>
              <Icon size={14} />{label}
            </Link>
          ))}
        </nav>
